...
directory_tensor = 'test/model_runs_test2/'
current_time = time.strftime("%Y%m%d-%H%M%S", time.localtime())
directory_tensor = os.path.join(directory_tensor, current_time)
writer = SummaryWriter(directory_tensor)
...
class PPOAgent:
    ...
    def lr_decay(self, ep):
        ...
        for p in self.critic_optimizer.param_groups:
            p['lr'] = lr_c_now
        # print('lr_a_now:', lr_a_now, 'lr_c_now:', lr_c_now)
    ...
    def update1(self, replay_buffer,ep):
        ...
        for _ in range(self.K_epochs):
            ...
            writer.add_scalar('Loss/entropy_loss',dist_entropy.mean(),global_step=self.num_actor_update_iteration)
            writer.add_scalar('Loss/ppo_loss',-torch.min(surr1, surr2).mean(),global_step=self.num_actor_update_iteration)
            writer.add_scalar('Loss/actor_loss',actor_loss.mean(),global_step=self.num_actor_update_iteration)
            ...
            writer.add_scalar('Loss/critic_loss',critic_loss.mean(),global_step=self.num_critic_update_iteration)
            # Update critic
...
for episode in range(max_episodes):
    ...
    while not done:
        ...
        # print(f'step {step},action {action},reward {reward},done {done},ter {ter}')
